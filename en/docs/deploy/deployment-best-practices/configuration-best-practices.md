...
**Loading**: Create a loader script that combines files in precedence order: secrets → environment-specific → base.

**Management**: All services use the same configuration loader mechanism. Update configurations in one place.
...
**Loading**: Each service's systemd file references its own configuration via `BAL_CONFIG_FILES`.

**Management**: Services deployed and configured independently. Good for teams owning individual services.

---

## Containerized Deployments

**Rule**: Do NOT include `Config.toml` in the container image.
...
**Precedence Management**: Leverage configuration precedence order correctly:
...
**Documentation**: Maintain endpoint and configuration documentation
...
**Auditability**: Log all configuration changes
- Who changed what
...
- [Ballerina Configuration Documentation](https://ballerina.io/learn/provide-values-to-configurable-variables/)
