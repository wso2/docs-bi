...
## Step 1 - Setup BI application for observability
...
* Enable Docker artifact generation by adding the following to the `build-options` in the `Ballerina.toml` file.
...
* Create a file named `Cloud.toml` in the project root directory and add the following content.
...
* In the `BI view`, under the `Deployment Options` select `Deploy on a VM` and select `Create Executable` option. This will create a Docker image `wso2inc/sample-service:v0.1.0`.

## Step 2 - Setup OpenSearch Deployment
...
Create `config/data-prepper/pipelines.yaml` to store the data prepper configurations.
...
Create `config/fluent-bit/fluent-bit.conf` and `config/fluent-bit/parsers.conf` to store the fluent bit configurations.

* `config/fluent-bit/fluent-bit.conf`

Collects, enriches, and routes Ballerina logs and metrics to OpenSearch for analysis.
...
Defines logfmt and JSON parsers to extract fields and timestamps from logs.
...
Enriches Ballerina logs with key fields and metadata for better analysis in OpenSearch.

```
function extract_app_from_path(tag, timestamp, record)
   if record["log_file_path"] then
      local path = record["log_file_path"]
      -- Extract application name from path like /var/log/ballerina/sample-service/app.log
...
OpenSearch dashboard and visualization configs for integration logs and metrics.
...
Create `config/dashboards/opensearch_dashboards.yml` to store the OpenSearch dashboard configurations.
...
* Create a `docker-compose.yml` file. This deploys a ready-to-use observability platform, with automated setup and a sample service for demonstration.
...
Open the OpenSearch Dashboard in your browser at `http://localhost:5601` and navigate to the `Traces` tab within the `Observability` section.
